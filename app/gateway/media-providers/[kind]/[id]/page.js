...
import ConnectionsCard from "@/app/gateway/providers/components/ConnectionsCard";
import ModelsCard from "@/app/gateway/providers/components/ModelsCard";
...
export default function MediaProviderDetailPage() {
  ...
  const handleDeleteCustom = async () => {
    ...
    try {
      ...
      if (res.ok) router.push(`/gateway/media-providers/${kind}`);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div>
        <Link
          href={`/gateway/media-providers/${kind}`}
