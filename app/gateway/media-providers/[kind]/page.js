...
function MediaProviderCard({ provider, kind, connections, isCustom, onToggle }) {
  ...
  return (
    <Link href={`/gateway/media-providers/${kind}/${provider.id}`} className="group">
      <Card
        padding="xs"
...
function ComboList({ combos }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      {combos.map((combo) => (
        <Link key={combo.id} href={`/gateway/media-providers/combo/${combo.id}`}>
...
export default function MediaProviderKindPage() {
  ...
  useEffect(() => {
    if (kind === "webSearch" || kind === "webFetch") {
      router.replace("/gateway/media-providers/web");
    }
  }, [kind, router]);
  ...
  const handleCreateCombo = async () => {
    ...
    if (res.ok) {
      ...
      router.push(`/gateway/media-providers/combo/${created.id}`);
    } else {
