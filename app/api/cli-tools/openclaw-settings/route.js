...
// Check if settings has Gateway config
const hasGatewayConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["gateway"];
};

// Read per-agent models.json and return current model id (without "gateway/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["gateway"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasGateway: hasGatewayConfig(settings),
...
// POST - Update Gateway settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `gateway/${model}`;

    // Remove all old gateway/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("gateway/"))
    ...
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`gateway/${m}`] = {};
    });

    // Remove old gateway model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("gateway/")) {
    ...
    // Update models.providers.gateway with all models
    settings.models.providers["gateway"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `gateway/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove Gateway settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Gateway from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["gateway"];
    ...
    // Remove gateway models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("gateway/"));
    ...
    // Reset agents.defaults.model.primary if it uses gateway
    if (settings.agents?.defaults?.model?.primary?.startsWith("gateway/")) {
