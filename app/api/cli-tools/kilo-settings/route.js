...
const hasGatewayConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["gateway"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("gateway");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasGateway: hasGatewayConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "Gateway", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Gateway settings removed from Kilo Code" });
  } catch (error) {
