...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["gateway"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasGateway: hasGatewayConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("gateway/") ? config.model.replace(/^gateway\//, "") : null,
...
// POST - Apply Gateway as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_gateway";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing gateway provider entry and its models
    const existingProvider = config.provider["gateway"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["gateway"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `gateway/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `gateway/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("gateway/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Gateway provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["gateway"]?.models) {
      delete config.provider["gateway"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["gateway"].models).length === 0) {
        delete config.provider["gateway"];
        if (config.model?.startsWith("gateway/")) delete config.model;
      } else if (config.model === `gateway/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["gateway"].models);
        config.model = `gateway/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire gateway provider
      if (config.provider) delete config.provider["gateway"];
      if (config.model?.startsWith("gateway/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("gateway/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Gateway settings removed from OpenCode",
    });
  } catch (error) {
