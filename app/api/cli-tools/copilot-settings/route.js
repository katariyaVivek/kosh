...
const hasGatewayConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Gateway");
};

const getGatewayEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Gateway") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getGatewayEntry(config);
...
// POST - Apply Gateway config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Gateway entry or append
    const idx = config.findIndex((e) => e.name === "Gateway");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Gateway entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Gateway");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Gateway removed from Copilot config",
    });
  } catch (error) {
