...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-gateway.env");
};
...
const hasGatewayConfig = (config) => {
  ...
  if (providers["gateway"]) return true;
...
export async function GET() {
  ...
  const hasGateway = hasGatewayConfig(config);
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile gateway",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "gateway configuration removed from jcode",
    });
  } catch (error) {
