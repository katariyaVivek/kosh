...
// Check if settings has Gateway customModels
const hasGatewayConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Gateway"));
};
...
// POST - Update Gateway customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Gateway configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Gateway"));
...
// DELETE - Remove Gateway customModels only (keep other settings)
