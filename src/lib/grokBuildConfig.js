...
const previousDefaultRegExp = /^# gateway-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# gateway-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via Gateway gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# gateway-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# gateway-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
