...
const getPageInfo = (pathname) => {
  ...
  // Media provider detail: /gateway/media-providers/[kind]/[id]
  ...
  if (mediaDetailMatch) {
    ...
    return {
      ...
      description: "",
      breadcrumbs: [
        { label: "Media Providers", href: `/gateway/media-providers/${kindId}` },
        { label: kindConfig?.label || kindId, href: `/gateway/media-providers/${kindId}` },
        { label: provider?.name || providerId, image: getProviderIconSrc(providerId) },
      ],
    };
  }

  // Media provider kind: /gateway/media-providers/[kind]
  ...
  // Provider detail page: /gateway/providers/[id]
  ...
  if (providerMatch) {
    ...
    if (providerInfo) {
      return {
        ...
        description: "",
        breadcrumbs: [
          { label: "Providers", href: "/gateway/providers" },
          {
  ...
  if (pathname.includes("/combos"))
    return {
      title: "Combos",
      description: "Model combos with fallback and vision adaptation",
      icon: "layers",
      breadcrumbs: [],
    };
  ...
  if (pathname.includes("/mitm"))
    return {
      title: "MITM Proxy",
      description: "Intercept CLI tool traffic and route through Gateway",
      icon: "security",
      breadcrumbs: [],
    };
  ...
  if (pathname.includes("/skills"))
    return {
      title: "Agent Skills",
      description: "Agent skills for routing and gateway access",
      icon: "extension",
      breadcrumbs: [],
    };
  ...
  return (
    <header className="shrink-0 flex items-center justify-between gap-3 px-4 lg:px-8 pt-3 pb-2 border-b border-border-subtle bg-surface/60 backdrop-blur-xl lg:bg-transparent lg:backdrop-blur-none z-20">
      ...
      <div className="flex flex-col min-w-0 flex-1">
            ...
            {breadcrumbs.map((crumb, index) => (
              ...
              >
                ...
                {crumb.href ? (
                  <Link
                    href={crumb.href}
                    className="text-xs text-text-muted hover:text-text-main transition-colors flex items-center gap-1.5"
                  >
                    {crumb.icon && (
                  ...
                  <div className="flex items-center gap-1.5">
                    {crumb.icon && (
                      <span className="material-symbols-outlined text-[16px] text-text-main">
                        {crumb.icon}
                      </span>
                    )}
                    {crumb.image && (
        ...
        {description && (
          <p className="text-xs text-text-muted mt-0.5">{translate(description)}</p>
        )}
      </div>
